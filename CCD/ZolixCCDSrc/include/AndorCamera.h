#pragma once
#include "CameraDll.h"
class CAndorCamera:public CCameraDll
{
//使用单例模式 限制每次每种相机只能连一个,现一台谱仪只有一个相机很合适
//单例模式规划还不算太好，几个了重复代码考虑使用继承或其他会更好
//后续修改
public: 
	static CAndorCamera * GetInstance(){
		if(spAndorCamera==NULL){
			spAndorCamera=new CAndorCamera();
		}
		return spAndorCamera;
	};   
private:
	CAndorCamera(){}; 
	static CAndorCamera * spAndorCamera;
	//回收指针
	class CGarbo 
	{
	public:
		~CGarbo()
		{ 
			if (CAndorCamera::spAndorCamera)
				delete CAndorCamera::spAndorCamera;
		}
	};
	static CGarbo Garbo;
public:
	...
	int GetGainNum();                                //no need
	void GetGain(int Index,float *fGain);            //
	int GetSpeedNum();                               //no need
	...
	void WriteAllIni();
	///状态控制 如需中断至FALSE
	void ImageDataAcquisition(long*plBuffer,BYTE* pBuffer,int XSize,int YSize);
	...
	void RemoveGhosting(long* plBuffer, int XSize, int YSize, const CString& ghostRegionStr);

private:
	BOOL gState;
};

extern CAndorCamera *pAndorCamera;